...
//    	model.addAttribute("unassignedProjects", babyService.getUnassignedUsers(userService.oneUser((Long) session.getAttribute("userId"))));
    ...
    public String processBaby(@Valid @ModelAttribute("newBaby") Baby newBaby, BindingResult result, Model model, HttpSession session) {
    	model.addAttribute("userId", session.getAttribute("userId"));
    	model.addAttribute("userName", session.getAttribute("userName"));
    	if(session.getAttribute("userId") == null) return "redirect:/";
    	if(result.hasErrors()) {
